...
class NetBoxDNSSource(octodns.provider.base.BaseProvider):
    ...
    def _get_nb_view(self, view: str | None | Literal[False]) -> dict[str, int | str]:
        ...
        nb_view: pynetbox.core.response.Record = self.api.plugins.netbox_dns.views.get(
            name=view
        )
        if nb_view is None:
    ...
    def _get_nb_zone(
        self, name: str, view: dict[str, str | int]
    ...
    def _format_rdata(
        self, rdata: dns.rdata.Rdata, raw_value: str
    ) -> str | dict[str, Any]:
        """
        ...
        match rdata.rdtype.name:
            ...
            case "SPF" | "TXT":
                value = raw_value.replace("\\\\", "\\").replace(";", r"\;")
    ...
    def _format_nb_records(self, zone: octodns.zone.Zone) -> list[dict[str, Any]]:
        ...
        nb_records: pynetbox.core.response.RecordSet = (
            self.api.plugins.netbox_dns.records.filter(zone_id=nb_zone.id)
        )
